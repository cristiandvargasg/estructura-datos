...
class Vehiculo:
    ...
    def encender(self):
        if self.combustible >= 10:
            ...
            print(f"Advertencia: La {self.tipo} necesita ir a la gasolinera. Debe tener minimo un 10% para arrancar.")
    ...
    def avanzar(self, distancia):
        if self.encendido:
            ...
            if self.combustible > consumo:
                ...
                print(f"La {self.tipo} no tiene suficiente combustible para avanzar.")
        else:
            print(f"La {self.tipo} está apagado, enciéndelo primero.")
...
carro = Carro("carro", "Honda", 30) 
moto = Moto("moto", "NKD", 9)
